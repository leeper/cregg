...
Additionally all functions have arguments in `data`-then-`formula` order, making it simple to pipe into them via `%>%`.
...
hainmueller$ethnosplit <- NA_real_
hainmueller$ethnosplit[hainmueller$ethnocentrism <= 10] <- 1L
hainmueller$ethnosplit[hainmueller$ethnocentrism > 10] <- 2L
hainmueller$ethnosplit <- factor(hainmueller$ethnosplit, 1:2, c("Low Ethnocentrism", "High Ethnocentrism"))
...
amces <- cj(na.omit(hainmueller), ChosenImmigrant ~ ReasonForApplication + LanguageSkills,
            id = ~ CaseID, estimate = "amce", by = ~ ethnosplit)
diff_amces <- cj(na.omit(hainmueller), ChosenImmigrant ~ ReasonForApplication + LanguageSkills,
                 id = ~ CaseID, estimate = "amce_diff", by = ~ ethnosplit)
amce_stacked <- rbind(amces[!names(amces) %in% c("z", "p")], diff_amces[!names(diff_amces) %in% c("t", "p")])
plot(amce_stacked) + ggplot2::facet_wrap(~BY, ncol = 3L)
```
...
# calculate conditional AMCEs
mms <- cj(na.omit(hainmueller), ChosenImmigrant ~ ReasonForApplication + LanguageSkills,
          id = ~ CaseID, estimate = "mm", by = ~ ethnosplit)
diff_mms <- cj(na.omit(hainmueller), ChosenImmigrant ~ ReasonForApplication + LanguageSkills,
               id = ~ CaseID, estimate = "mm_diff", by = ~ ethnosplit)
mm_stacked <- rbind(mms[!names(mms) %in% c("z", "p")], diff_mms[!names(diff_mms) %in% c("t", "p")])
plot(mm_stacked) + ggplot2::facet_wrap(~BY, ncol = 3L)
```
